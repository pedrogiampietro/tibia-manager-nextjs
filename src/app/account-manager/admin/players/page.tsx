...
export default function AdminManagerPlayer() {
  ...
  const [data, setData] = useState<{ totalPages: number, players: any[] }>({
    totalPages: 0,
    players: []
  });
      ...
      <div className="flex flex-col rounded-sm border">
        ...
        <Table>
          ...
          <TableBody>
            ...
            {!isLoading && data?.players?.map((player) => (
              <TableRow key={player.id.toString()}>
                <TableCell>{player.id.toString().padStart(6, '0')}</TableCell>
                <TableCell>{player.name}</TableCell>
                <TableCell className="text-right w-[30px]"><ActionsDropdown /></TableCell>
              </TableRow>
